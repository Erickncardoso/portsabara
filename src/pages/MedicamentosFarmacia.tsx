...
import { Search, Plus, Filter, AlertTriangle, Package, Pill, ArrowDown, ArrowUp } from 'lucide-react';
...
const MedicamentosFarmacia: React.FC = () => {
  ...
  const medicamentosEmEstoque = medicamentos.filter(m => m.estoque > 0).length;
  ...
  const onSubmit = (data: MedicamentoFormData) => {
    const novoMedicamento = {
      ...data,
      status: data.estoque === 0 ? 'Esgotado' : 
              data.estoque < data.estoqueMinimo ? 'Baixo estoque' : 
              'Disponível'
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-white rounded-xl shadow p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-800">{totalMedicamentos}</p>
                </div>
              ...
              <div className="bg-white rounded-xl shadow p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-yellow-600">{medicamentosBaixoEstoque}</p>
                </div>
              ...
              <div className="bg-white rounded-xl shadow p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">{medicamentosEmEstoque}</p>
                </div>
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-blue-50">
                      <th className="px-4 py-3 text-left font-semibold text-blue-700">Nome</th>
                      <th className="px-4 py-3 text-left font-semibold text-blue-700">Categoria</th>
                      <th className="px-4 py-3 text-left font-semibold text-blue-700">Estoque</th>
                      <th className="px-4 py-3 text-left font-semibold text-blue-700">Validade</th>
                      <th className="px-4 py-3 text-left font-semibold text-blue-700">Status</th>
                      <th className="px-4 py-3 text-left font-semibold text-blue-700">Fornecedor</th>
                      <th className="px-4 py-3 text-left font-semibold text-blue-700">Lote</th>
                      <th className="px-4 py-3 text-left font-semibold text-blue-700">Preço</th>
                    ...
                    {filteredMedicamentos.map((m, idx) => (
                      <tr key={idx} className="border-b hover:bg-blue-50">
                        ...
                        <td className={`px-4 py-3 font-semibold ${
                          m.status === 'Esgotado' ? 'text-red-600' : 
                          m.status === 'Baixo estoque' ? 'text-yellow-600' : 
                          'text-green-600'
                        }`}>
                          {m.status}
                        </td>
                        <td className="px-4 py-3">{m.fornecedor}</td>
                        <td className="px-4 py-3 font-mono text-xs">{m.lote}</td>
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        />
                        {errors.estoqueMinimo && <p className="text-red-500 text-xs mt-1">{errors.estoqueMinimo.message}</p>}
                      </div>
                    </div>
