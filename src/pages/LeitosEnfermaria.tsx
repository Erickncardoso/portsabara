...
const LeitosEnfermaria = () => {
  ...
  const leitos = [
    ...
    { numero: '104', status: 'Ocupado', paciente: 'Maria Santos', tipo: 'Enfermaria' },
  ];
                ...
                <TableBody>
                  {leitos.map((leito) => (
                    <TableRow key={leito.numero}>
                      <TableCell className="font-medium">{leito.numero}</TableCell>
                      <TableCell>
                        <span className={cn(
                          "px-2 py-1 rounded-full text-xs",
                          leito.status === 'Ocupado' ? "bg-red-100 text-red-800" :
                          leito.status === 'Livre' ? "bg-green-100 text-green-800" :
