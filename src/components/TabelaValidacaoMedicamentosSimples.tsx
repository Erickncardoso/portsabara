...
const TabelaValidacaoMedicamentosSimples: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const medicamentosArmazenados = localStorage.getItem('medicamentosValidacaoEnfermaria');
    ...
    if (medicamentosArmazenados) {
      ...
      localStorage.setItem('medicamentosValidacaoEnfermaria', JSON.stringify(dadosIniciais));
    }
  }, []);

  const salvarMedicamentos = (novosMedicamentos: Medicamento[]) => {
    setMedicamentos(novosMedicamentos);
    localStorage.setItem('medicamentosValidacaoEnfermaria', JSON.stringify(novosMedicamentos));
  };
  ...
  return (
    <Card className="shadow-md border border-gray-100 hover:shadow-lg transition-shadow duration-300 bg-white">
      <CardHeader className="bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-t-lg pb-3 pt-4">
        <CardTitle className="text-lg font-semibold">Validação de Medicamentos</CardTitle>
      </CardHeader>
      
      <CardContent className="p-0">
        <div className="divide-y divide-gray-100">
          {medicamentos.map((medicamento) => (
            <div key={medicamento.id} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-center gap-3">
                <Avatar className="h-9 w-9 border-2 border-orange-100">
                  <AvatarImage src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${medicamento.avatar}`} />
                  <AvatarFallback className="bg-orange-100 text-orange-700">{medicamento.paciente.charAt(0)}</AvatarFallback>
                </Avatar>
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-xs text-gray-500">{medicamento.data}</span>
                  </div>
                </div>
              </div>
