...
const TabelaConsultas: React.FC<TabelaConsultasProps> = ({ consultas: consultasIniciais, tipo, titulo }) => {
  ...
  const recusarConsulta = (id: string) => {
    if (tipo === 'proximas') {
      ...
      toast({
        title: "Alta recusada",
        description: `Paciente ${consultas.find(c => c.id === id)?.paciente} permanece internado`,
        variant: "destructive"
      });
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg border p-3 shadow-md hover:shadow-lg transition-all h-auto">
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="py-1 text-xs">{tipo === 'proximas' ? 'Aprovação' : 'Alta'}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {consultas.length === 0 ? (
            ...
            consultas.map((consulta) => (
              <TableRow key={consulta.id} className={consulta.aprovada === false ? 'bg-red-50' : consulta.aprovada ? 'bg-green-50' : ''}>
                <TableCell className="py-1 text-xs">
                  <div className="flex items-center gap-1">
                    <Avatar className="h-5 w-5">
                      <AvatarImage src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${consulta.paciente}`} alt={consulta.paciente} />
                      <AvatarFallback>{consulta.paciente.substring(0, 2)}</AvatarFallback>
                    </Avatar>
                    {consulta.paciente}
                  </div>
                </TableCell>
                <TableCell className="py-1 text-xs">{consulta.doenca}</TableCell>
      ...
      <div className="text-right mt-1">
        <a href="#" className="text-blue-500 text-xs hover:underline">Ver mais...</a>
      </div>
    </div>
  );
};
